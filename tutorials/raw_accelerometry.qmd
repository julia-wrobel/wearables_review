...
```{r sleep_ck}
ck = actigraph.sleepr::apply_sadeh(data)
head(ck)
```
...
```
Most of these highlighted areas are probably *not* sleep, which emphasizes the need for plotting the data and carefully choosing a sleep detection algorithm. We could impose an additional filter, such as requiring that sleep periods are at least one hour long: 

```{r}
sleep_periods_small = 
  sleep_periods %>% 
  filter(sleep == "S") %>% 
  mutate(length = as.numeric(difftime(period_end, period_start, unit = "mins"))) %>% 
  filter(length >= 60)
  
data %>% 
  ggplot(aes(x = timestamp, y = ac)) + 
  geom_line() +
  geom_rect(data = sleep_periods_small,
            aes(xmin = period_start,
                xmax = period_end,
                ymin = -Inf,
                ymax = Inf),
...
Also note that the nonwear period is scored as sleep by this algorithm, so in practice, we should remove nonwear periods first before applying the sleep detection algorithm. 
